...
# Only these hosts may be framed. Subdomains count, so www. and player. are covered.
EMBED_HOSTS = ('youtube.com', 'youtu.be', 'vimeo.com')
...
def embed_src(value):
    ...
    if path.startswith('/video/'):
        return url
    video = path.lstrip('/').split('/')[0]
    return f'https://player.vimeo.com/video/{video}' if video.isdigit() else ''


def _youtube(video):
    return f'https://www.youtube.com/embed/{video}' if _VIDEO_ID.match(video or '') else ''
...
def _safe_url(value, allow_remote=False):
    ...
    if url.startswith('/'):
        return url
    if allow_remote and url.lower().startswith(('http://', 'https://')) and _host_allowed(url):
        return url
    return ''


def _safe_link(value):
    """A URL fit for an href: site-relative, or http(s) on any host. Never a script scheme."""
    url = (value or '').strip()
    if not url or url.startswith('//'):
        return ''
    if url.startswith('/') or url.lower().startswith(('http://', 'https://')):
        return url
    return ''
...
def sanitize_document(value):
    ...
    for block in value.get('blocks') or []:
        ...
        if data is not None:
            supplied = block.get('id') or ''
            block_id = supplied if _SAFE_BLOCK_ID.match(supplied) else _block_id()
