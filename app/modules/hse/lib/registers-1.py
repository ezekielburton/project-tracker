...
from app.modules.hse.lib.registers import HSE_REGISTERS, register, registers_in_group
from app.modules.hse.lib.table import columns, status_chips, table_rows
from app.modules.hse.routes.blueprint import hse_bp


def _first_built():
    """The first register that exists, for the module's bare URL."""
    return HSE_REGISTERS[0] if HSE_REGISTERS else None
...
def index():
    ...
    if reg is None:
        abort(404)
    return redirect(url_for('hse.register_page', group_key=reg.group, register_key=reg.key))
