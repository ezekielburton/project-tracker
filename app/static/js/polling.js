...
(function () {
    ...
    // Last time we hit /api/version, so the redeploy check can be debounced
    // (see init()). Module-level so it persists across navigations.
    ...
    function init() {
        ...
        // Redeploy check, debounced. The stamp only changes when the server
        // process restarts, so checking it on every navigation just spends a
        // connection slot during rapid nav (part of the quick-nav hang). Once
        // per 10s is plenty to catch a deploy.
        var _now = Date.now();
